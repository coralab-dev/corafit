...
## Current task state
- Editor stale-save/results follow-up (2026-07-10): exercise draft tracker now blocks archive for invalid, pending, or failed prescription saves; archive dialog surfaces the failure. Keyed exercise mutation queues clean only the latest completed entry. Exercise library requests use AbortController/latest-request identity, hide stale errors, and disable selection/pagination while refreshing. No backend, Prisma, Supabase, migration, seed, or API contract changes.
- Editor stabilization follow-up (2026-07-10): structural mutations now use a real FIFO queue with per-action results and recovery after failures; archiving saves drafts, blurs the active editor, waits for pending mutations, and prevents duplicate requests; exercise search uses server pagination with exclusion-aware empty-page messaging. Added web regression tests and `pnpm --filter web test`. No backend, Prisma, Supabase, migration, seed, API contract, plan library, or assigned-plan editor changes.
- Editor de planes Athletic Warm (2026-07-10): `/training-plans/[planId]/edit` se reorganizó con cabecera compacta y acción por contexto, estado global de guardado + Ctrl/Cmd+S, publicación guiada, árbol controlado y responsive, alta de días training/rest, sesión como superficie principal, selector modal reutilizando `ExerciseSearch`, tabla/cards de prescripciones con validación y confirmaciones Radix. Sin cambios de API/Prisma/Supabase ni del editor de planes asignados. Pruebas puras, web typecheck/lint/build y diff check ejecutados; QA visual autenticado pendiente porque Browser no expuso runtime y el repo no incluye Playwright.
- Biblioteca de planes de entrenamiento (2026-07-10): el buscador se presenta como búsqueda por nombre porque el endpoint remoto sólo aplica `search` sobre `name`. El filtro de origen (`Plantillas`/`Mis planes`) sigue siendo local sobre los elementos cargados (máximo 50 por consulta); queda como deuda futura añadir paginación/filtrado server-side o un agregado por origen para cubrir toda la biblioteca sin cambiar el alcance actual.
- COR-1003A billing minimo beta completed locally: six zero-price beta plans live in `packages/db/prisma/backfill-beta-subscriptions.ts`; backfill upserts all plans and assigns Trial only to orgs without subscriptions. `registerProfile` keeps Trial canonical. `/billing/current` returns legacy fields plus `clientUsage`; client creation no longer blocks on `clientLimit` but still requires active subscription. Settings shows usage and soft at/over-limit copy; admin organizations shows usage ratio and manual review badge. Verification passed: api typecheck/lint/test, web typecheck/lint, db typecheck/lint, and direct backfill spec. No data/backfill execution, Supabase, deploy, payments, checkout, manual plan change, or billing admin UI work.
- Admin organizations UI on 2026-07-07: added `/admin/organizations` as the beta operation screen for `admin_saas`, with search/status filters, organization list, read-only detail, and link to `/admin/exercises`. New hook `useAdminOrganizations` consumes `GET /admin/organizations` and `GET /admin/organizations/:organizationId` with `authenticatedRequest(..., { session })` only, so no `organizationId` / `X-Organization-Id` is sent. Nav `Admin` now points to organizations, and admin-without-org redirect goes there. Verification passed: `pnpm --config.verify-deps-before-run=false --filter web typecheck` and `pnpm --config.verify-deps-before-run=false --filter web lint`. No API, Prisma, seeds, billing, data, payments, Supabase, or deploy changes.
